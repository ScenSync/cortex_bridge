...
use crate::{debug, error, info, warn, set_error_msg, c_str_to_string, CortexNetworkInfo, CortexWebClient};
use tokio;
use gethostname;
use url;
...
static WEB_CLIENT_INSTANCES: Lazy<Mutex<HashMap<String, (Arc<WebClient>, tokio::runtime::Runtime)>>> =
...
pub extern "C" fn cortex_start_web_client(client_config: *const CortexWebClient) -> c_int {
    ...
    let runtime = match tokio::runtime::Runtime::new() {
        ...
        Err(e) => {
            error!("cortex_start_web_client: Failed to create tokio runtime: {}", e);
    ...
    let result = runtime.block_on(async {
        ...
        info!("cortex_start_web_client: Base URL: {}, Token: {}", base_url, token);
        ...
        // Create connector
        let connector = match create_connector_by_url(base_url.as_str(), &global_ctx, IpVersion::Both).await {
            Ok(conn) => {
    ...
    match result {
        Ok((web_client, instance_name)) => {
            ...
            info!("cortex_start_web_client: Web client instance '{}' registered", instance_name);
            0
        },
        Err(e) => {
            error!("cortex_start_web_client: Failed to create web client instance: {}", e);
...
pub extern "C" fn cortex_stop_web_client(instance_name: *const c_char) -> c_int {
    let name = match c_str_to_string(instance_name) {
        Ok(name) => {
            info!("cortex_stop_web_client: Stopping web client instance: {}", name);
            name
        },
        Err(e) => {
    ...
    if let Some((_web_client, _runtime)) = instances.remove(&name) {
        debug!("cortex_stop_web_client: Found web client instance: {}", name);
        // WebClient and Runtime will be dropped automatically
        info!("cortex_stop_web_client: Web client instance '{}' stopped", name);
        0
    } else {
        warn!("cortex_stop_web_client: Web client instance '{}' not found", name);
...
pub extern "C" fn cortex_get_web_client_network_info(
    ...
    let name = match c_str_to_string(instance_name) {
        Ok(name) => {
            debug!("cortex_get_web_client_network_info: Getting network info for instance: {}", name);
            name
        },
        Err(e) => {
            error!("cortex_get_web_client_network_info: Invalid instance_name: {}", e);
            set_error_msg(&format!("invalid instance_name: {}", e));
            return -1;
        }
    };
    ...
    let network_name_c = match CString::new(network_name) {
        Ok(s) => s.into_raw(),
        Err(_) => {
            unsafe { let _ = CString::from_raw(instance_name_c); }
    ...
    info!("cortex_get_web_client_network_info: Successfully created network info for instance: {}", name);
    0
}
    ...
    for i in 0..count {
        c_strings.push(match CString::new(instance_names[i].clone()) {
            ...
            Err(_) => {
                ...
                for j in 0..i {
                    unsafe { let _ = CString::from_raw(c_strings[j]); }
                }
                set_error_msg("failed to convert instance name to C string");
                return -1;
            }
