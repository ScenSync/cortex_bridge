...
impl From<HeartbeatRequest> for SerializableHeartbeatRequest {
    fn from(req: HeartbeatRequest) -> Self {
        SerializableHeartbeatRequest {
            ...
            running_network_instances: req.running_network_instances.into_iter().map(|id| id.to_string()).collect(),
        }
    }
}
...
impl NetworkConfigService {
    ...
    pub async fn new(db_url: &str, geoip_path: Option<String>) -> Result<Self> {
        let client_mgr = ClientManager::new(db_url, geoip_path).await
            .map_err(|e| anyhow::anyhow!("Failed to create ClientManager: {:?}", e))?;
        
        Ok(Self { client_mgr: Arc::new(client_mgr) })
    }
    ...
    pub async fn start(&mut self, protocol: &str, port: u16) -> Result<()> {
        ...
        client_mgr.start(protocol, port).await
    ...
    async fn get_session_by_device_id(
        ...
        let Some(result) = self.client_mgr.get_session_by_device_id(user_id, device_id).await else {
    ...
    pub async fn run_network_instance(
        ...
        // Direct database operation - update device network config
        {
            ...
            if let Some(existing) = existing_device {
                ...
                crate::info!("Device not found in database, creating new device record: {}", device_id);
                // 创建新的设备记录
        ...
        crate::info!("Checking if network instance {} is running before extracting virtual IP", inst_id);
        match self.check_network_instance_running(org_id, device_id, &inst_id).await {
            Ok(true) => {
                crate::info!("Network instance {} is running, extracting virtual IP", inst_id);
                match self.update_device_virtual_ip(org_id, device_id, &inst_id).await {
                    Ok(_) => crate::info!("Virtual IP update completed successfully for device {}", device_id),
                    Err(e) => crate::error!("Virtual IP update failed for device {}: {:?}", device_id, e),
                }
            },
            Ok(false) => {
                crate::warn!("Network instance {} is not running, skipping virtual IP extraction", inst_id);
            },
            Err(e) => {
    ...
    pub async fn list_network_instance_ids(
        ...
        let disabled_inst_ids = {
            ...
            devices::Entity::find()
                ...
                .await?
                .iter()
                .filter_map(|x| x.network_instance_id.as_ref().and_then(|id| uuid::Uuid::parse_str(id).ok()))
                .collect::<Vec<_>>()
        };
    ...
    pub async fn list_devices(
        ...
        let client_urls = self.client_mgr.list_devices_by_organization_id(user_id).await;
        ...
        if disabled {
            ...
            c.run_network_instance(
                ...
                RunNetworkInstanceRequest {
                    ...
                    config: Some(serde_json::from_value(device.network_config.as_ref().unwrap().clone())?),
                },
            )
            .await
            .map_err(convert_rpc_error)?;
    ...
    pub async fn get_network_config(
        ...
        let config = serde_json::from_value::<NetworkConfig>(device.network_config.as_ref().unwrap().clone())
    ...
    pub async fn batch_update_network_state(
        ...
        for inst_id in inst_ids {
            let result = self.update_network_state(user_id, device_id, &inst_id, disabled).await;
            results.push(result);
        }
    ...
    pub async fn batch_remove_network_instances(
        ...
        for inst_id in inst_ids {
            let result = self.remove_network_instance(user_id, device_id, &inst_id).await;
            results.push(result);
        }
    ...
    async fn check_network_instance_running(
        ...
        let network_info = self.collect_one_network_info(org_id, device_id, inst_id).await?;
        ...
        crate::debug!("Network instance {} running status: {}", inst_id_str, is_running);
        Ok(is_running)
    }
    ...
    async fn update_device_virtual_ip(
        ...
        for attempt in 1..=3 {
            crate::info!("Attempt {}: Collecting network info for device {} instance {}", attempt, device_id, inst_id);
            ...
            crate::info!("Attempt {}: Extracting virtual IP from network info for device {}", attempt, device_id);
            if let Some(virtual_ip_info) = self.extract_virtual_ip_from_network_info(&network_info, inst_id) {
                crate::info!("Found virtual IP: {}/{} for device {}", virtual_ip_info.0, virtual_ip_info.1, device_id);
                // Update device virtual IP directly in database
                self.update_device_virtual_ip_in_db(device_id, virtual_ip_info.0, virtual_ip_info.1).await?;
                crate::info!("Successfully updated device {} virtual IP: {}/{}", device_id, virtual_ip_info.0, virtual_ip_info.1);
                return Ok(());
            } else {
                crate::warn!("Attempt {}: No virtual IP found in network info for device {}", attempt, device_id);
                // Debug: Print the network info structure to understand what we're getting
                crate::debug!("Attempt {}: Network info structure: {}", attempt, serde_json::to_string_pretty(&network_info).unwrap_or_default());
        ...
        crate::error!("Failed to extract virtual IP after 3 attempts for device {}", device_id);
        Ok(())
    }
    ...
    fn extract_virtual_ip_from_network_info(
        ...
        let running = virtual_ip_info.get("running").and_then(|r| r.as_bool()).unwrap_or(false);
        if !running {
            crate::warn!("Network instance {} is not running, skipping virtual IP extraction", inst_id_str);
            return None;
        }
        ...
        crate::info!("Successfully extracted virtual IP: {}/{}", addr, network_length);
    ...
    async fn update_device_virtual_ip_in_db(
        ...
        if let Some(existing_device) = device {
            ...
            active_model.update(db.orm()).await?;
            crate::info!("Updated device {} virtual IP in database: {}/{}", device_id, virtual_ip, network_length);
        } else {
