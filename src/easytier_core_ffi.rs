...
use crate::{CLIENT_INSTANCES, set_error_msg, c_str_to_string};
...
pub extern "C" fn start_easytier_core(core_config: *const EasyTierCoreConfig) -> c_int {
    ...
    let instance_name = match c_str_to_string(config.instance_name) {
        Ok(name) => {
            info!("start_easytier_core: Instance name received from Go: '{}'", name);
            name
        },
        Err(e) => {
    ...
    let network_name = match c_str_to_string(config.network_name) {
        Ok(name) => {
            info!("start_easytier_core: Network name received from Go: '{}'", name);
            name
        },
        Err(e) => {
    ...
    let network_secret = match c_str_to_string(config.network_secret) {
        Ok(secret) => {
            info!("start_easytier_core: Network secret received from Go: '{}' (length: {})", secret, secret.len());
            secret
        },
        Err(e) => {
    ...
    let operation_mode = if config.private_mode != 0 {
        ...
        info!("start_easytier_core: Starting in P2P mode with network: {}", network_name);
        "p2p"
    } else {
    ...
    info!("start_easytier_core: RPC port from config: {}", config.rpc_port);
    info!("start_easytier_core: Listener URLs count: {}", config.listener_urls_count);
    
    // Parse peer URLs (for P2P mode)
    if config.peer_urls_count > 0 && !config.peer_urls.is_null() {
        debug!("start_easytier_core: Parsing {} peer URLs", config.peer_urls_count);
    ...
    match operation_mode {
        "p2p" => {
            if peer_urls.is_empty() {
                ...
                info!("start_easytier_core: P2P mode - {} peers configured", peer_urls.len());
            }
        },
        "private" => {
        ...
        _ => {
            error!("start_easytier_core: Unknown operation mode: {}", operation_mode);
    ...
    if listener_urls.len() < 2 {
        error!("start_easytier_core: Insufficient listener URLs provided (need at least 2, got {})", listener_urls.len());
        set_error_msg(&format!("insufficient listener URLs provided (need at least 2, got {})", listener_urls.len()));
        return -1;
    }
...
{}"#,
                instance_name,
                dhcp,
                if ipv4.is_empty() { String::new() } else { format!("\nipv4 = \"{}\"", ipv4) },
                if ipv6.is_empty() { String::new() } else { format!("\nipv6 = \"{}\"", ipv6) },
                final_listeners.iter().map(|l| format!("    \"{}\"", l)).collect::<Vec<_>>().join(",\n"),
                config.rpc_port,
                network_name,
                network_secret,
                default_protocol,
...
"#,
        ...
        _ => {
            error!("start_easytier_core: Unknown operation mode for TOML generation: {}", operation_mode);
            set_error_msg(&format!("unknown operation mode for TOML generation: {}", operation_mode));
            return -1;
        }
    };
    info!("start_easytier_core: Generated TOML configuration:\n{}", toml_config);
    debug!("start_easytier_core: TOML configuration created with {} peer URLs", all_peer_urls.len());
    info!("start_easytier_core: RPC port used in TOML: {}", config.rpc_port);
    info!("start_easytier_core: RPC portal format: 0.0.0.0:{}", config.rpc_port);
    ...
    match instance.start() {
        Ok(_event_subscriber) => {
            ...
            if let Ok(mut instances) = CLIENT_INSTANCES.lock() {
                ...
                info!("start_easytier_core: Core instance '{}' registered and started successfully", instance_name);
            } else {
        ...
        Err(e) => {
            error!("start_easytier_core: Failed to start network instance: {}", e);
...
pub extern "C" fn stop_easytier_core(instance_name: *const c_char) -> c_int {
    ...
    if let Ok(mut instances) = CLIENT_INSTANCES.lock() {
        if let Some(_instance_id) = instances.remove(&name) {
            ...
            info!("stop_easytier_core: Core instance '{}' stopped successfully", name);
            0
        } else {
