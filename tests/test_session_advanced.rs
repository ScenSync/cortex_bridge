...
use easytier::proto::{web::{HeartbeatRequest, RunNetworkInstanceRequest, NetworkConfig}, common::Uuid as ProtoUuid};
use url::Url;
...
async fn test_session_data_creation() {
    let db = get_test_database("test_session_data_creation").await
    ...
    assert!(session_data.req().is_none(), "Initial request should be None");
    assert!(session_data.location().is_none(), "Initial location should be None");
    
    // 删除测试数据库
    remove_test_database("test_session_data_creation").await
...
async fn test_session_heartbeat_waiter_functionality() {
    let db = get_test_database("test_session_heartbeat_waiter_functionality").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    // 删除测试数据库
    remove_test_database("test_session_heartbeat_waiter_functionality").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_session_rpc_client_creation() {
    let db = get_test_database("test_session_rpc_client_creation").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(rpc_client.is_some(), "Should be able to create RPC client even when session is not running");
    
    // 删除测试数据库
    remove_test_database("test_session_rpc_client_creation").await
        .expect("Failed to remove test database");
}
...
async fn test_session_network_instance_operations() {
    ...
    let db = get_test_database("test_session_network_instance_operations").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(run_result.is_err(), "Should fail when no RPC connection is available");
    
    // Test stopping network instance
    let stop_result = session.stop_network_instance("test_instance".to_string()).await;
    assert!(stop_result.is_err(), "Should fail when no RPC connection is available");
    
    // Test listing network instances
    let list_result = session.list_network_instances().await;
    assert!(list_result.is_err(), "Should fail when no RPC connection is available");
    
    // 删除测试数据库
    remove_test_database("test_session_network_instance_operations").await
        .expect("Failed to remove test database");
}
...
async fn test_location_edge_cases() {
    ...
    for location in locations {
        ...
        assert!(json_result.is_ok(), "Location should be serializable: {:?}", location);
        
        let json_str = json_result.unwrap();
        let deserialized_result: Result<Location, _> = serde_json::from_str(&json_str);
        assert!(deserialized_result.is_ok(), "Location should be deserializable: {}", json_str);
...
async fn test_session_debug_formatting() {
    let db = get_test_database("test_session_debug_formatting").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(debug_str.contains("Session"), "Debug output should contain 'Session'");
    assert!(!debug_str.is_empty(), "Debug output should not be empty");
    
    // 删除测试数据库
    remove_test_database("test_session_debug_formatting").await
        .expect("Failed to remove test database");
}
...
async fn test_storage_token_debug_and_clone() {
    ...
    assert!(debug_str.contains("StorageToken"), "Debug output should contain 'StorageToken'");
    assert!(debug_str.contains(&token), "Debug output should contain token");
...
async fn test_concurrent_session_operations() {
    let db = get_test_database("test_concurrent_session_operations").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    for handle in handles {
        ...
        assert!(result.is_ok(), "Concurrent session operations should succeed");
        let (task_id, token_is_none, is_running) = result.unwrap();
        assert!(token_is_none, "Token should be None for task {}", task_id);
        assert!(!is_running, "Session should not be running for task {}", task_id);
    }
    remove_test_database("test_concurrent_session_operations").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_session_data_req_functionality() {
    let db = get_test_database("test_session_data_req_functionality").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    // 删除测试数据库
    remove_test_database("test_session_data_req_functionality").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_url_edge_cases_for_sessions() {
    let db = get_test_database("test_url_edge_cases_for_sessions").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    for url_str in test_urls {
        ...
        assert!(!session.is_running(), "Session should not be running initially for URL: {}", url_str);
        
        let token = session.get_token().await;
        assert!(token.is_none(), "Token should be None initially for URL: {}", url_str);
    }
    remove_test_database("test_url_edge_cases_for_sessions").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_storage_weak_ref_debug() {
    let db = get_test_database("test_storage_weak_ref_debug").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(debug_str.contains("Weak"), "Debug output should contain 'Weak'");
    assert!(!debug_str.is_empty(), "Debug output should not be empty");
    
    // Test Clone for WeakRefStorage
    let cloned_weak = weak_storage.clone();
    let cloned_debug_str = format!("{:?}", cloned_weak);
    assert!(!cloned_debug_str.is_empty(), "Cloned weak reference should have debug output");
    remove_test_database("test_storage_weak_ref_debug").await
        .expect("Failed to remove test database");
}
