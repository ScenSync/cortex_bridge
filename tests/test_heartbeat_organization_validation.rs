...
async fn test_heartbeat_with_valid_organization() {
    let db = get_test_database("test_heartbeat_with_valid_organization").await
    ...
    let _heartbeat_request = HeartbeatRequest {
        machine_id: Some(ProtoUuid {
            ...
            part4: u32::from_be_bytes([uuid_bytes[12], uuid_bytes[13], uuid_bytes[14], uuid_bytes[15]]),
        }),
        inst_id: Some({
            let inst_uuid = Uuid::new_v4();
            let inst_uuid_bytes = inst_uuid.as_bytes();
            ProtoUuid {
                part1: u32::from_be_bytes([inst_uuid_bytes[0], inst_uuid_bytes[1], inst_uuid_bytes[2], inst_uuid_bytes[3]]),
                part2: u32::from_be_bytes([inst_uuid_bytes[4], inst_uuid_bytes[5], inst_uuid_bytes[6], inst_uuid_bytes[7]]),
                part3: u32::from_be_bytes([inst_uuid_bytes[8], inst_uuid_bytes[9], inst_uuid_bytes[10], inst_uuid_bytes[11]]),
                part4: u32::from_be_bytes([inst_uuid_bytes[12], inst_uuid_bytes[13], inst_uuid_bytes[14], inst_uuid_bytes[15]]),
            }
        }),
    ...
    assert!(session.data().read().await.req().is_none(), "Session should start with no heartbeat request");
    
    // Verify organization exists in database for validation
    let org_exists = db.orm().execute(Statement::from_sql_and_values(
            DatabaseBackend::MySql,
        ...
        .await
        .is_ok();
    assert!(org_exists, "Organization should exist in database for validation");
    remove_test_database("test_heartbeat_with_valid_organization").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_heartbeat_with_invalid_organization() {
    let db = get_test_database("test_heartbeat_with_invalid_organization").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let org_count_result = db.orm().execute(Statement::from_sql_and_values(
            DatabaseBackend::MySql,
            "SELECT COUNT(*) as count FROM organizations WHERE id = ?",
            vec![invalid_org_id.into()]
        ))
        .await;
    assert!(org_count_result.is_ok(), "Query should execute successfully");
    remove_test_database("test_heartbeat_with_invalid_organization").await
        .expect("Failed to cleanup test database");
...
async fn test_heartbeat_organization_validation_multiple_requests() {
    let db = get_test_database("test_heartbeat_organization_validation_multiple_requests").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    for org_id in &valid_orgs {
        let org_exists = db.orm().execute(Statement::from_sql_and_values(
            ...
            .await
            .is_ok();
        assert!(org_exists, "Organization {} should exist in database for validation", org_id);
    }
    ...
    for org_id in &invalid_orgs {
        let org_count_result = db.orm().execute(Statement::from_sql_and_values(
            ...
            .await;
        assert!(org_count_result.is_ok(), "Query should execute successfully for invalid org {}", org_id);
        // In a real scenario, this would validate that the organization doesn't exist
    }
    remove_test_database("test_heartbeat_organization_validation_multiple_requests").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_heartbeat_with_location_and_organization_validation() {
    let db = get_test_database("test_heartbeat_with_location_and_organization_validation").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(stored_location.is_some(), "Session should have location data");
    
    let stored_location = stored_location.unwrap();
    assert_eq!(stored_location.country, location.country, "Country should match");
    assert_eq!(stored_location.city, location.city, "City should match");
    assert_eq!(stored_location.region, location.region, "Region should match");
    remove_test_database("test_heartbeat_with_location_and_organization_validation").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_heartbeat_organization_validation_with_udp_tunnel() {
    let db = get_test_database("test_heartbeat_organization_validation_with_udp_tunnel").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    client_manager.add_listener(Box::new(listener)).await.unwrap();
    ...
    assert!(heartbeat_available, "Heartbeat request should be available from UDP tunnel client");
    ...
    assert!(!heartbeat_req.user_token.is_empty(), "User token (organization ID) should be present in heartbeat");
    
    // Verify location information for local UDP connection
    let location = client_manager.get_device_location(&client_url).await;
    assert!(location.is_some(), "Location should be available for UDP connections");
    
    let loc = location.unwrap();
    println!("UDP client location: {:?}", loc);
    assert_eq!(loc.country, "本地网络", "Local network should be detected for UDP tunnel");
    ...
    remove_test_database("test_heartbeat_organization_validation_with_udp_tunnel").await
        .expect("Failed to remove test database");
}
...
async fn test_heartbeat_multiple_udp_clients() {
    let db = get_test_database("test_heartbeat_multiple_udp_clients").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let mut client_manager = ClientManager::new(&db_url, None)
        .await
        .expect("Failed to create ClientManager");
    client_manager.add_listener(Box::new(listener1)).await.unwrap();
    client_manager.add_listener(Box::new(listener2)).await.unwrap();
    ...
    for session_token in &sessions {
        ...
        if heartbeat_available {
            let heartbeat_req = client_manager.get_heartbeat_requests(&client_url).await.unwrap();
            heartbeat_count += 1;
            org_tokens.push(heartbeat_req.user_token.clone());
            println!("Client {} heartbeat: {:?}", heartbeat_count, heartbeat_req);
            assert!(!heartbeat_req.user_token.is_empty(), "User token should be present");
        }
    }
    
    assert!(heartbeat_count > 0, "Should receive heartbeat requests from UDP tunnel clients");
    ...
    remove_test_database("test_heartbeat_multiple_udp_clients").await
        .expect("Failed to remove test database");
}