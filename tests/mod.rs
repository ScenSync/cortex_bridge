...
use common::*;
    ...
    async fn test_database_connection() {
        let db = get_test_database("test_database_connection").await
    ...
    async fn test_org_operations() {
        let db = get_test_database("test_org_operations").await
            .expect("Failed to setup test database");
        cleanup_test_database(&db).await
            .expect("Failed to cleanup test database");
        ...
        assert!(insert_result.is_ok(), "Organization creation should succeed");
        
        // Query the created organization
        let query_result = db.orm()
        ...
        assert_eq!(description, Some("A test organization for unit testing".to_string()));
    ...
    async fn test_device_operations() {
        let db = get_test_database("test_device_operations").await
            .expect("Failed to setup test database");
        cleanup_test_database(&db).await
            .expect("Failed to cleanup test database");
    ...
    async fn test_client_manager_initialization() {
        ...
        let _client_manager = ClientManager::new(&db_url, None).await
    ...
    async fn test_client_manager_sessions() {
        let db = get_test_database("test_client_manager_sessions").await
            .expect("Failed to setup test database");
        cleanup_test_database(&db).await
            .expect("Failed to cleanup test database");
        ...
        // Test ClientManager creation with new signature
        let client_manager = ClientManager::new(&db_url, None).await
            .expect("Failed to create ClientManager");
    ...
    async fn test_session_creation() {
        let db = get_test_database("test_session_creation").await
            .expect("Failed to setup test database");
        cleanup_test_database(&db).await
            .expect("Failed to cleanup test database");
