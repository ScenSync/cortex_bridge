...
use easytier_bridge::db::Database;
use std::sync::Arc;
...
#[allow(dead_code)]
pub async fn get_test_database(test_function_name: &str) -> Result<Database, Box<dyn std::error::Error>> {
    let db_name = create_test_db_name(test_function_name);
    ...
    let db = Database::new_for_test(&db_url).await
...
async fn create_database_if_not_exists(db_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    use sea_orm::{Database as SeaOrmDatabase, ConnectionTrait, Statement, DatabaseBackend};
    
    let conn = SeaOrmDatabase::connect(BASE_DB_URL).await
        .map_err(|e| format!("Failed to connect to MySQL server: {}", e))?;
    
    // Create database if not exists
    let create_db_query = format!("CREATE DATABASE IF NOT EXISTS `{}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci", db_name);
...
async fn create_tables_if_not_exist(db: &Database) -> Result<(), Box<dyn std::error::Error>> {
    ...
    use easytier_bridge::db::migrations::Migrator;
...
#[allow(dead_code)]
pub async fn create_test_organization(db: &Database, org_id: &str) -> Result<(), Box<dyn std::error::Error>> {
    use sea_orm::{ConnectionTrait, Statement, DatabaseBackend};
...
macro_rules! setup_test_db {
    () => {{
        let test_name = std::thread::current().name()
            ...
            .unwrap_or("unknown_test")
            .to_string();
        let db = $crate::common::get_test_database(&test_name).await
            .expect("Failed to setup test database");
        $crate::common::cleanup_test_database(&db).await
...
macro_rules! setup_test_db_with_org {
    () => {{
        ...
        $crate::common::cleanup_test_database(&db).await
            .expect("Failed to cleanup test database");
        let org_id = $crate::common::setup_test_organization(&db).await
...
#[allow(dead_code)]
pub async fn remove_test_database(test_function_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    use sea_orm::{Database as SeaOrmDatabase, ConnectionTrait, Statement, DatabaseBackend};
