...
async fn test_client_manager_initialization() {
    let db = get_test_database("test_client_manager_initialization").await
    ...
    // 删除测试数据库
    remove_test_database("test_client_manager_initialization").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_client_manager_running_state() {
    let db = get_test_database("test_client_manager_running_state").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(!is_running, "ClientManager should not be running without listeners");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_client_manager_running_state").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_empty_session_list() {
    let db = get_test_database("test_empty_session_list").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(sessions.is_empty(), "Session list should be empty initially");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_empty_session_list").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_session_by_machine_id() {
    let db = get_test_database("test_session_by_machine_id").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(session.is_none(), "Session should not exist for non-existent machine");
    
    // // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_session_by_machine_id").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_list_machines_by_org_empty() {
    let db = get_test_database("test_list_machines_by_org_empty").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let machines = client_manager.list_devices_by_organization_id(&org_id).await;
    
    assert!(machines.is_empty(), "Machine list should be empty for user with no machines");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_list_machines_by_org_empty").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_list_devices_by_organization_with_clients() {
    let db = get_test_database("test_list_devices_by_organization_with_clients").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert_eq!(machines.len(), 3, "Should return 3 devices for the organization");
    
    // Verify all expected URLs are present (order may vary)
    for expected_url in &expected_urls {
        assert!(machines.contains(expected_url), 
               "Should contain expected device URL: {}", expected_url);
    }
    ...
    // 删除测试数据库
    remove_test_database("test_list_devices_by_organization_with_clients").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_list_devices_by_organization_different_orgs() {
    let db = get_test_database("test_list_devices_by_organization_different_orgs").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let user1_machines = client_manager.list_devices_by_organization_id(&org_id_1).await;
    let user2_machines = client_manager.list_devices_by_organization_id(&org_id_2).await;
    
    assert_eq!(user1_machines.len(), 2, "Organization 1 should have 2 devices");
    assert_eq!(user2_machines.len(), 3, "Organization 2 should have 3 devices");
    
    // Verify no overlap between organizations' devices
    for machine1 in &user1_machines {
        assert!(!user2_machines.contains(machine1), 
               "Organization 1's device should not appear in organization 2's list: {}", machine1);
    }
    ...
    // 删除测试数据库
    remove_test_database("test_list_devices_by_organization_different_orgs").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_list_devices_by_organization_inactive_filtered() {
    let db = get_test_database("test_list_devices_by_organization_inactive_filtered").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert_eq!(machines[0].to_string(), active_client_url, "Should return the active device URL");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_list_devices_by_organization_inactive_filtered").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_list_devices_by_organization_database_error_handling() {
    let db = get_test_database("test_list_devices_by_organization_database_error").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let machines = client_manager.list_devices_by_organization_id(invalid_user_id).await;
    
    // Should return empty list without crashing
    assert!(machines.is_empty(), "Should return empty list for invalid organization ID");
    
    // Test with empty organization ID
    let empty_user_id = "";
    let machines = client_manager.list_devices_by_organization_id(empty_user_id).await;
    
    assert!(machines.is_empty(), "Should return empty list for empty organization ID");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_list_devices_by_organization_database_error").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_heartbeat_requests() {
    let db = get_test_database("test_heartbeat_requests").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(heartbeat.is_none(), "Heartbeat should be None for non-existent client");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_heartbeat_requests").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_machine_location() {
    let db = get_test_database("test_machine_location").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    assert!(location.is_none(), "Location should be None for non-existent client");
    
    // Cleanup resources
    client_manager.shutdown().await;
    
    // 删除测试数据库
    remove_test_database("test_machine_location").await
        .expect("Failed to remove test database");
}
...
async fn test_database_access() {
    ...
    let db = get_test_database("test_database_access").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let result = db.orm().query_one(Statement::from_string(
            DatabaseBackend::MySql,
    ...
    // 删除测试数据库
    remove_test_database("test_database_access").await
        .expect("Failed to remove test database");
}
...
async fn test_private_ip_location_lookup() {
    ...
    for url_str in private_urls {
        ...
        assert!(_private_url.host_str().is_some(), "Private URL should have valid host");
    }
}

#[tokio::test]
async fn test_geoip_integration() {
    let db = get_test_database("test_geoip_integration").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    // 删除测试数据库
    remove_test_database("test_geoip_integration").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_concurrent_session_access() {
    let db = get_test_database("test_concurrent_session_access").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    
    let db_url = get_test_database_url("test_concurrent_session_access");
    let client_manager = Arc::new(ClientManager::new(&db_url, None).await
        .expect("Failed to create ClientManager"));
    ...
    for handle in handles {
        ...
        assert_eq!(result.unwrap(), 0, "All concurrent calls should return empty list");
    }
    ...
    // 删除测试数据库
    remove_test_database("test_concurrent_session_access").await
        .expect("Failed to remove test database");
}