...
extern "C" {
    ...
    fn destroy_network_config_service_singleton(
        err_msg: *mut *mut c_char,
    ) -> bool;
...
fn test_ffi_geoip_explicit_path() {
    ...
    assert!(geoip_path.is_some(), "Should be able to auto-detect GeoIP path for testing");
...
fn test_ffi_geoip_multiple_creations() {
    ...
    unsafe {
        // First creation
        let result1 = create_network_config_service_singleton(
            c_db_url,
            ptr::null(),
            &mut err_msg,
        );
        ...
        if !result1 {
            ...
            let result2 = create_network_config_service_singleton(
                c_db_url,
                ptr::null(),
                &mut err_msg,
            );
            ...
            assert!(result2, "Second creation should return true (singleton already exists)");
...
fn test_ffi_geoip_actual_file_exists() {
    ...
    assert!(metadata.len() > 1000, "GeoIP database file should be larger than 1KB");
...
fn test_ffi_geoip_environment_variable() {
    ...
    if let Some(path) = &geoip_path {
        ...
        let env_path = std::env::var("CORTEX_GEOIP_DB_PATH").expect("Environment variable should be set");
        assert_eq!(env_path, *path, "Environment variable should match the path");
        ...
        unsafe {
            ...
            if !result {
                println!("NetworkConfigService creation failed as expected with environment variable");
                if !err_msg.is_null() {
                    free_c_char(err_msg);
                }
            } else {
