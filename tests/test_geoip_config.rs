...
async fn test_geoip_config_auto_detection() {
    ...
    assert!(geoip_path.is_some(), "GeoIP database path should be auto-detected");
    
    let path = geoip_path.unwrap();
    println!("Auto-detected GeoIP path: {}", path);
    
    // Verify the file actually exists
    assert!(Path::new(&path).exists(), "GeoIP database file should exist at: {}", path);
}

#[tokio::test]
async fn test_client_manager_with_auto_geoip() {
    let db = get_test_database("test_client_manager_with_auto_geoip").await
    ...
    let mut client_manager = ClientManager::new(&db_url, None).await
        .expect("Failed to create ClientManager");
    
    // ClientManager should be created successfully
    assert!(!client_manager.is_running(), "ClientManager should not be running initially");
    
    // Cleanup resources
    client_manager.shutdown().await;
    remove_test_database("test_client_manager_with_auto_geoip").await
        .expect("Failed to remove test database");
}

#[tokio::test]
async fn test_client_manager_with_explicit_geoip() {
    let db = get_test_database("test_client_manager_with_explicit_geoip").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let geoip_path = get_geoip_db_path();
    assert!(geoip_path.is_some(), "Should be able to auto-detect GeoIP path");
    
    // Create ClientManager with explicit GeoIP path
    let db_url = get_test_database_url("test_client_manager_with_explicit_geoip");
    let mut client_manager = ClientManager::new(&db_url, geoip_path).await
        .expect("Failed to create ClientManager");
    ...
    // Cleanup resources
    client_manager.shutdown().await;
    remove_test_database("test_client_manager_with_explicit_geoip").await
        .expect("Failed to remove test database");
}
...
fn test_geoip_path_fallback() {
    ...
    assert_eq!(path, Some("/non/existent/path/geoip2-cn.mmdb".to_string()), "Should return env var path even if file doesn't exist");
    ...
    assert!(fallback_path.is_some(), "Should fallback to auto-detection when env var is not set");
}
...
async fn test_concurrent_geoip_access() {
    ...
    let db = get_test_database("test_concurrent_geoip_access").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    for i in 0..5 {
        tasks.spawn(async move {
            ...
            let mut client_manager = ClientManager::new(&db_url, None).await
                .expect("Failed to create ClientManager");
            assert!(!client_manager.is_running(), "ClientManager {} should not be running initially", i);
            client_manager.shutdown().await;
            i
        });
    }
    ...
    while let Some(result) = tasks.join_next().await {
        assert!(result.is_ok(), "Concurrent ClientManager creation should succeed");
        completed += 1;
    }
    
    assert_eq!(completed, 5, "All concurrent tasks should complete successfully");
    remove_test_database("test_concurrent_geoip_access").await
        .expect("Failed to remove test database");
}
...
fn test_geoip_path_unicode_characters() {
    ...
    for test_path in test_cases {
        ...
        let path = get_geoip_db_path();
        assert_eq!(path, Some(test_path.to_string()), "Should handle Unicode characters in path");
    }
...
async fn test_client_manager_resource_cleanup() {
    let db = get_test_database("test_client_manager_resource_cleanup").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    remove_test_database("test_client_manager_resource_cleanup").await
        .expect("Failed to remove test database");
}
...
fn test_geoip_config_thread_safety() {
    ...
    assert_eq!(counter.load(Ordering::Relaxed), 10, "All threads should complete successfully");
}
...
fn test_geoip_path_edge_cases() {
    ...
    let test_cases = vec![
        ("/some/absolute/path/geoip.mmdb", "Absolute path should be returned as-is"),
        ("~", "Tilde should be returned as-is"),
        ("$HOME/geoip.mmdb", "Environment variable in path should be returned as-is"),
        ("/valid/path/geoip.mmdb", "Valid path should be returned as-is"),
        ("relative/path/geoip.mmdb", "Relative path should be returned as-is"),
    ];
    ...
    assert!(empty_path.is_some(), "Empty string should trigger fallback to auto-detection");
    assert_ne!(empty_path, Some("".to_string()), "Should not return empty string");
    if let Some(path) = empty_path {
        assert!(path.ends_with("geoip2-cn.mmdb"), "Should find the actual GeoIP database file");
    }
...
async fn test_client_manager_with_none_and_some_geoip() {
    let db1 = get_test_database("test_client_manager_none_geoip1").await
        .expect("Failed to setup test database 1");
    let db2 = get_test_database("test_client_manager_some_geoip2").await
        .expect("Failed to setup test database 2");
    
    cleanup_test_database(&db1).await
        .expect("Failed to cleanup test database 1");
    cleanup_test_database(&db2).await
    ...
    let mut client_manager1 = ClientManager::new(&db_url1, None).await
    ...
    let mut client_manager2 = ClientManager::new(&db_url2, geoip_path).await
        .expect("Failed to create ClientManager 2");
    
    // Both should work
    assert!(!client_manager1.is_running(), "ClientManager with None should not be running initially");
    assert!(!client_manager2.is_running(), "ClientManager with Some should not be running initially");
    
    // Cleanup resources
    client_manager1.shutdown().await;
    client_manager2.shutdown().await;

    remove_test_database("test_client_manager_none_geoip1").await
        .expect("Failed to remove test database 1");
    remove_test_database("test_client_manager_some_geoip2").await
...
fn test_geoip_path_empty_env_var() {
    ...
    assert!(path.is_some(), "Should fallback to auto-detection when env var is empty");
    
    // The path should not be empty
    if let Some(p) = path {
        assert!(!p.is_empty(), "Detected path should not be empty");
        assert!(p.contains("geoip2-cn.mmdb"), "Should detect the correct GeoIP database file");
    }
    ...
    assert!(auto_path.is_some(), "Auto-detection should find the GeoIP database");
    if let Some(p) = auto_path {
        assert!(p.contains("geoip2-cn.mmdb"), "Auto-detected path should be the GeoIP database");
    }
}

#[tokio::test]
async fn test_client_manager_with_invalid_geoip_path() {
    let db = get_test_database("test_client_manager_with_invalid_geoip_path").await
        .expect("Failed to setup test database");
    cleanup_test_database(&db).await
        .expect("Failed to cleanup test database");
    ...
    let mut client_manager = ClientManager::new(&db_url, invalid_path).await
        .expect("Failed to create ClientManager");
    ...
    // Cleanup resources
    client_manager.shutdown().await;
    remove_test_database("test_client_manager_with_invalid_geoip_path").await
        .expect("Failed to remove test database");
}
...
fn test_geoip_path_with_relative_paths() {
    ...
    for test_path in test_cases {
        std::env::set_var("CORTEX_GEOIP_DB_PATH", test_path);
        let path = get_geoip_db_path();
        assert_eq!(path, Some(test_path.to_string()), "Should return the exact environment variable value");
    }
...
async fn test_multiple_client_managers_with_same_geoip() {
    let db1 = get_test_database("test_multiple_client_managers_1").await
        .expect("Failed to setup test database 1");
    let db2 = get_test_database("test_multiple_client_managers_2").await
        .expect("Failed to setup test database 2");
    ...
    let mut client_manager1 = ClientManager::new(&db_url1, geoip_path.clone()).await
        .expect("Failed to create ClientManager 1");
    let mut client_manager2 = ClientManager::new(&db_url2, geoip_path).await
        .expect("Failed to create ClientManager 2");
    
    // Both should be created successfully
    assert!(!client_manager1.is_running(), "ClientManager 1 should not be running initially");
    assert!(!client_manager2.is_running(), "ClientManager 2 should not be running initially");
    
    // Cleanup resources
    client_manager1.shutdown().await;
    client_manager2.shutdown().await;
    remove_test_database("test_multiple_client_managers_1").await
        .expect("Failed to remove test database 1");
    remove_test_database("test_multiple_client_managers_2").await
        .expect("Failed to remove test database 2");
}
...
fn test_geoip_path_environment_variable_priority() {
    ...
    let path = get_geoip_db_path();
    assert_eq!(path, Some(custom_path.to_string()), "Environment variable should take precedence");
    ...
    let auto_path = get_geoip_db_path();
    if let Some(p) = auto_path {
        assert_ne!(p, custom_path, "Auto-detected path should be different from custom path");
        assert!(p.contains("geoip2-cn.mmdb"), "Auto-detected path should contain the database filename");
    }
}
...
fn test_geoip_path_special_characters() {
    ...
    for test_path in test_cases {
        std::env::set_var("CORTEX_GEOIP_DB_PATH", test_path);
        let path = get_geoip_db_path();
        assert_eq!(path, Some(test_path.to_string()), "Should handle special characters in path: {}", test_path);
    }
