...
impl Session {
    ...
    async fn run_network_on_start(
        ...
        loop {
            ...
            // Step 1: Check if device is approved and has network config (ONE network per device)
            let device = {
                use crate::db::entities::devices;
                use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};

                // Get device with network config
                    ...
                    .await
                {
                    Ok(Some(d)) => d,
                    Ok(None) => {
                        crate::warn!("[run_network_on_start] Device {} not found", device_id);
                        return;
                    }
                    Err(e) => {
                        crate::error!("[run_network_on_start] Failed to query device: {:?}", e);
                        return;
                    }
                }
            };
            ...
            if !device.status.is_approved() {
                match device.status {
                    crate::db::entities::devices::DeviceStatus::Pending => {
                        crate::info!("[run_network_on_start] Device {} is pending approval, no networks will be started", device_id);
                    }
                    crate::db::entities::devices::DeviceStatus::Rejected => {
                        crate::warn!("[run_network_on_start] Device {} is rejected, no networks will be started", device_id);
                    }
                    _ => {
                        crate::debug!("[run_network_on_start] Device {} status is {:?}, not starting networks", device_id, device.status);
                    }
                }
                return;
            }
            ...
            let network_config_opt = if device.network_disabled == Some(true) {
                crate::info!(
                    "[run_network_on_start] Device {} has disabled network, skipping",
                    device_id
                );
                None
            ...
            {
                crate::debug!(
                    "[run_network_on_start] Found network config for device {}",
                    device_id
                );
                Some((inst_id.clone(), config_json.clone()))
            } else {
                crate::debug!(
                    "[run_network_on_start] Device {} has no network configuration",
                    device_id
                );
                None
            };

            // Process network configuration if exists
            if let Some((instance_id, config_json)) = network_config_opt {
                crate::debug!(
                    "[run_network_on_start] Processing network config: instance_id={}",
                    instance_id
                );
                ...
                // Parse network config from JSON
                let network_config: NetworkConfig = match serde_json::from_value(config_json) {
                    Ok(config) => {
                    ...
                    Err(e) => {
                        crate::error!(
                                "[run_network_on_start] Failed to parse network config for instance {}: {:?}",
                                instance_id,
                                e
                            );
                        return;
                ...
                crate::debug!(
                    "[run_network_on_start] No network configuration to start for device {}",
                    device_id
                );
                break;
            }
