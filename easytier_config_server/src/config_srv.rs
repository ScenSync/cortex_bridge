...
impl NetworkConfigService {
    ...
    pub async fn run_network_instance(
        ...
        // Update device with network configuration (ONE network per device)
        {
            ...
            crate::info!("Updating network instance for device {}", device_id);

            // Update device with network configuration
            ...
            active_model.network_create_time = Set(Some(Utc::now().into()));
            active_model.network_update_time = Set(Some(Utc::now().into()));

            active_model.update(db.orm()).await?;
            crate::info!(
                "Successfully updated network instance {} in database",
                inst_id
            );
        }
    ...
    pub async fn list_network_instance_ids(
        ...
        // Collect disabled networks from devices table (ONE network per device)
        let db = self.client_mgr.db().await;
        let disabled_inst_ids = {
            ...
            match device {
                Some(d) if d.network_disabled == Some(true) => d
                    .network_instance_id
    ...
    pub async fn remove_network_instance(
        ...
        let db = self.client_mgr.db().await;
        // Clear network configuration from devices table
        {
            use crate::db::entities::devices;
            ...
            let device = devices::Entity::find()
                ...
                .one(db.orm())
                .await?
                .ok_or_else(|| anyhow::anyhow!("Device with network instance not found"))?;
            ...
            active_model.virtual_ip = Set(None);
            active_model.virtual_ip_network_length = Set(None);

            active_model.update(db.orm()).await?;

            crate::info!(
                "Cleared network instance {} from device {}",
                inst_id,
                device_id
            );
        }
    ...
    pub async fn update_network_state(
        ...
        let network_config = {
            ...
            let device = devices::Entity::find()
                ...
                .one(db.orm())
                .await?
                .ok_or_else(|| anyhow::anyhow!("Network instance not found"))?;
    ...
    async fn update_device_virtual_ip(
        ...
        for attempt in 1..=3 {
                ...
                self.extract_virtual_ip_from_network_info(&network_info, inst_id)
            {
                ...
                // Update network virtual IP in database
    ...
    async fn update_device_virtual_ip_in_db(
        ...
        let device = devices::Entity::find()
            ...
            .await?
            .ok_or_else(|| {
                anyhow::anyhow!("Device with network instance not found: {}", inst_id)
            })?;
        ...
        active_model.update(db.orm()).await?;
        crate::info!(
            "Updated virtual IP for network {} on device {}: {}/{}",
            inst_id,
            device_id,
            virtual_ip,
            network_length
